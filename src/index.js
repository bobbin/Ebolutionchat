...
export default class ReactBotUI extends Component {
  constructor(props) {
    ...
    this.state = {
      title: props.title || "Ebolution Bot",
      messages: [],
      isBotTyping: false,
      happyOrNot: true,
  ...
  processResponse(text) {
    const messages = text.match(/[^.!?]+[.!?]*/g).map(str => str.trim());
  ...
  getResponse(text) {
    return this.dialogflow.textRequest(text).then(data => {
      if (data.result.action === "input.unknown") {
        console.log(data.result);
        this.setState({ happyOrNot: false });
      } else {
        this.setState({ happyOrNot: true });
      }
  ...
  handleSubmitText(text) {
    ...
    if (this.dialogflow) {
      this.getResponse(text).then(this.processResponse);
    } else if (this.props.getResponse) {
      this.props.getResponse(text).then(this.processResponse);
    } else {
      this.processResponse("Lo siento, aún no estoy 100% configurado. :'(");
    }
  }
      ...
      >
        ...
        >
          <Dialog
            ...
            isHappyOrNot={this.state.happyOrNot}
